...
function fuelauConfiguredServices(): array
{
    return [
        ...
        'photon' => [
            'service' => 'photon',
            'title' => 'Photon',
            'role' => 'Default Australia autocomplete geocoder with a versioned local index.',
            'kind' => 'runtime',
            'profile' => 'routing',
            'expected_state' => 'running when routing profile is enabled',
            'expected_detail' => 'Expected to run from the latest validated Australia index.',
            'start_command' => 'docker compose --profile routing up -d photon',
            'data_paths' => ['var/docker/photon-eval'],
            'artifact_checks' => ['var/docker/photon-eval/current/photon_data'],
            'source' => 'https://download1.graphhopper.com/public/australia-oceania/australia/',
        ],
        'photon-refresh' => [
            'service' => 'photon-refresh',
            'title' => 'Photon Refresh',
            'role' => 'One-shot verified Australia snapshot and atomic Photon index refresh.',
            'kind' => 'setup_job',
            'profile' => 'photon-setup',
            'expected_state' => 'exited successfully or prepared',
            'expected_detail' => 'Runs weekly through photon-scheduler or manually on demand.',
            'start_command' => 'docker compose --profile photon-setup run --rm photon-refresh',
            'data_paths' => ['var/docker/photon-input', 'var/docker/photon-eval'],
            'artifact_checks' => ['var/docker/photon-eval/current/photon_data'],
        ],
        'photon-scheduler' => [
            'service' => 'photon-scheduler',
            'title' => 'Photon Scheduler',
            'role' => 'Weekly Docker CLI scheduler for verified Photon Australia updates.',
            'kind' => 'runtime',
            'profile' => 'routing',
            'expected_state' => 'running when routing profile is enabled',
            'expected_detail' => 'Checks for a changed official snapshot each Monday at 02:15 Brisbane time.',
            'start_command' => 'docker compose --profile routing up -d photon-scheduler',
            'data_paths' => ['var/docker/app-logs', 'var/docker/photon-input', 'var/docker/photon-eval'],
            'artifact_checks' => ['var/docker/photon-eval/current/photon_data'],
        ],
        'osrm-download' => [
