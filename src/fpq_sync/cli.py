...
def fetch_json(api_host: str, token: str, path: str) -> dict[str, object]:
    ...
    try:
        with retry_urlopen(request, timeout=60) as response:
...
def stage_live_data(mysql_env_path: str, api_host: str, token: str) -> tuple[str, int, int]:
    ...
    sql_chunks = [
        """
DELETE FROM `fpq_stage_sites`
WHERE `created_at` < UTC_TIMESTAMP() - INTERVAL 7 DAY;
DELETE FROM `fpq_stage_prices`
WHERE `created_at` < UTC_TIMESTAMP() - INTERVAL 7 DAY;
""".strip(),
        build_stage_sites_sql(sync_batch_id, sites),
        build_stage_prices_sql(sync_batch_id, prices),
    ]
    sql = "\n\n".join(chunk for chunk in sql_chunks if chunk)
    if sql:
        run_mysql_sql(mysql_env_path, sql)
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_job_name = f"fpq_{args.job.replace('-', '_')}"
    ...
    if args.job not in ("diagnose", "stage", "analyze"):
        try:
