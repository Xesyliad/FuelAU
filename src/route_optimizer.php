...
final readonly class FuelauOptimizerPolicy
{
    public function __construct(
        ...
        // Retained for request compatibility; these legacy values are ignored.
        public int $minimumStopSpacingM = 0,
        public int $minimumStopSpacingS = 0,
...
final readonly class FuelauOptimizerPlan
{
    ...
    public function __construct(
        ...
        public int $fuelOnlyStopCount = 0,
        public int $combinedStopCount = 0,
    ) {}
}
...
final class FuelauFuelStateOptimizer
{
    ...
    public function optimizePractical(
        ...
        $maximumStops = $policy->maximumFuelOnlyStops
            ?? ($minimumStopPlan->fuelOnlyStopCount + $automaticAllowance);
        if ($maximumStops < $minimumStopPlan->fuelOnlyStopCount) {
        ...
        for ($stabilizationPass = 0; $stabilizationPass < count($nodes); $stabilizationPass++) {
            for ($constraintPass = 0; $constraintPass < count($nodes); $constraintPass++) {
                ...
                foreach ($plan->purchases as $purchase) {
                    $combinedStop = $nodesById[$purchase->nodeId]->combinedStop ?? false;
                    ...
                    $tooSmall = !$combinedStop && $purchase->purchaseL < $minimumPurchaseL;
                    ...
                    if ($tooSmall || $tooFar || $beyondSafetyDetour) {
                        ...
                        if ($alternative === null) {
                            ...
                            $required[$purchase->nodeId] = match (true) {
                                ...
                                default => 'detour_safety_override',
                            };
                        } elseif (
                            !$tooFar
                            && $alternative->fuelOnlyStopCount >= $plan->fuelOnlyStopCount
                        ) {
                            $required[$purchase->nodeId] = $tooSmall
                                ? 'minimum_purchase_safety_override'
                                : 'detour_safety_override';
                        } else {
            ...
            for ($auditPass = 0; $auditPass < count($nodes); $auditPass++) {
                $changed = false;
                foreach ($plan->purchases as $purchase) {
                    if ($nodesById[$purchase->nodeId]->combinedStop ?? false) {
                        continue;
                    }
                    if (isset($required[$purchase->nodeId])) {
                        continue;
                    ...
                    if ($alternative->fuelOnlyStopCount >= $plan->fuelOnlyStopCount) {
        ...
        for ($fromIndex = 0; $fromIndex < $lastIndex; $fromIndex++) {
            foreach ($states[$fromIndex] as $fromKey => $state) {
                ...
                foreach ($departureOptions as $departureBuckets) {
                    ...
                    $isCombinedStop = $purchaseBuckets > 0
                        && $nodes[$fromIndex]->combinedStop;
                    $nextStopCount = (int) $state['stop_count']
                        + ($purchaseBuckets > 0 ? 1 : 0);
                    $nextFuelOnlyStopCount = (int) $state['fuel_only_stop_count']
                        + ($purchaseBuckets > 0 && !$isCombinedStop ? 1 : 0);
                    if (
                        $maximumStops !== null
                        && $nextFuelOnlyStopCount > $maximumStops
                    ) {
                        continue;
                    }
                    ...
                    $purchaseGeneralizedCostUnits = (int) $state['generalized_cost_units']
                        + ($purchaseBuckets * (int) ($price ?? 0))
                        + ($purchaseBuckets > 0 && !$isCombinedStop
                            ? $stopCostCents * 20
                            : 0);
                    ...
                    for ($toIndex = $fromIndex + 1; $toIndex <= $lastIndex; $toIndex++) {
                        ...
                        $nextKey = $this->stateKey(
                            $nextFuelBuckets,
                            $nextFuelOnlyStopCount,
                        );
                        $existing = $states[$toIndex][$nextKey] ?? null;
                        if (
                            $existing !== null
                            && $this->preferExistingState(
                                $existing,
                                $nextStopCount,
                                $nextFuelCostUnits,
                                $nextGeneralizedCostUnits,
                                $objective,
                            )
                        ) {
                            continue;
                        ...
                        $states[$toIndex][$nextKey] = [
                            ...
                            'fuel_only_stop_count' => $nextFuelOnlyStopCount,
    ...
    ): string
    {
        ...
        return "{$fuelBuckets}:{$fuelOnlyStopCount}";
    }

    /**
     * @param array<string, int|string> $existing
     */
    private function preferExistingState(
        array $existing,
        int $candidateStopCount,
        int $candidateFuelCostUnits,
        int $candidateGeneralizedCostUnits,
        string $objective,
    ): bool {
        $existingStopCount = (int) $existing['stop_count'];
        $existingFuelCostUnits = (int) $existing['fuel_cost_units'];
        $existingGeneralizedCostUnits = (int) $existing['generalized_cost_units'];

        if ($objective === 'fewest_stops') {
            return (
                [$existingStopCount, $existingGeneralizedCostUnits, $existingFuelCostUnits]
                <=> [$candidateStopCount, $candidateGeneralizedCostUnits, $candidateFuelCostUnits]
            ) <= 0;
        }
        if ($objective === 'fuel_cost') {
            return (
                [$existingFuelCostUnits, $existingStopCount, $existingGeneralizedCostUnits]
                <=> [$candidateFuelCostUnits, $candidateStopCount, $candidateGeneralizedCostUnits]
            ) <= 0;
        }

        return (
            [$existingGeneralizedCostUnits, $existingStopCount, $existingFuelCostUnits]
            <=> [$candidateGeneralizedCostUnits, $candidateStopCount, $candidateFuelCostUnits]
        ) <= 0;
    }
    ...
    ): string
    {
        ...
        usort($keys, static function (string $leftKey, string $rightKey) use (
            ...
            $primary = match ($objective) {
                'fewest_stops' =>
                    (int) $left['fuel_only_stop_count']
                        <=> (int) $right['fuel_only_stop_count'],
            ...
            $secondary = $objective === 'fewest_stops'
                ? (
                    ...
                    ?: ((int) $left['generalized_cost_units']
                        <=> (int) $right['generalized_cost_units'])
                )
                : (
                    ((int) $left['fuel_only_stop_count']
                        <=> (int) $right['fuel_only_stop_count'])
    ...
    private function buildPlan(
        ...
        return new FuelauOptimizerPlan(
            ...
            fuelOnlyStopCount: (int) $destinationState['fuel_only_stop_count'],
            combinedStopCount: (int) $destinationState['stop_count']
                - (int) $destinationState['fuel_only_stop_count'],
        );
    }
    ...
    private function classifyPlan(
        ...
        $purchases = array_map(
            static function (FuelauOptimizerPurchase $purchase) use (
                ...
                return new FuelauOptimizerPurchase(
                    ...
                    marginalNetSavingCents: !$combined && $requiredReason === null
        ...
        return new FuelauOptimizerPlan(
            ...
            discretionaryStopCount: $plan->fuelOnlyStopCount - $requiredStopCount,
            fuelOnlyStopCount: $plan->fuelOnlyStopCount,
            combinedStopCount: $plan->combinedStopCount,
        );
    }
}
